...
def addAndLink(request):
    ...
    # 从表关联主表方式
    good.g_custom.add(cust)

    # 主表关联从表方式
    # cust.goods_set.add(good)
    return HttpResponse('添加并关联')
```
...
```
def queryKey(request):
    cust = Custom.objects.filter(c_name='赵四')[0]
    goods_list = cust.goods_set.all()
    for i in goods_list:
        print(i.id, i.g_name)
...
```
def queryFor(request):
    good = Goods.objects.filter(g_name='统一绿茶')[0]
    cust_list = good.g_custom.all()
    for i in cust_list:
        print(i.id, i.c_name)
...
删除从表数据，那么第三张表的数据也会删除（级联删除）

```
def deleteFor(request):
    good = Goods.objects.filter(g_name='统一绿茶')[0]
    good.delete()
    return HttpResponse('我要扇我自己')
```
...
def deleteKey(request):
    cust = Custom.objects.filter(c_name='刘能')[0]
    cust.delete()
    return HttpResponse('我要删你')
```
